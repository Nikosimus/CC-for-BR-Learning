...
def byz_marina_no_sync(filename, x_init, A, y, clients_A, clients_y, gamma, num_of_byz, p, num_of_workers,
    ...
    # Method
    while stopping_criterion(num_of_bits_passes, T):

        x = x - gamma * G_w1

        # Emulate the workers' behavior and aggregate their updates on-the-fly
        ...
        for i in range(num_of_workers - num_of_byz):
            ...
            if (bernoulli_arr[bernoulli_counter] == 1):
                ...
                new_grad = logreg_grad(deepcopy(x), [A_i, y_i, l2, setting])
                grads[i] = new_grad
                grad_diff = new_grad - logreg_grad(x1, [A_i, y_i, l2, setting])
        ...
        if attack == "BF":
            for i in range(num_of_workers - num_of_byz, num_of_workers):
                ...
                if (bernoulli_arr[bernoulli_counter] == 1):
                    ...
                    grad_diff = logreg_grad(deepcopy(x), [A_i, y_i, l2, setting]) - logreg_grad(x1, [A_i, y_i, l2, setting])
                    G_w[i] -= sparsificator(grad_diff, sparsificator_params)[0]
        ...
        if attack == "LMP":
            for i in range(num_of_workers - num_of_byz, num_of_workers):
                ...
                y_i = clients_y[i]
                if (bernoulli_arr[bernoulli_counter] == 1):
                    w[i-(num_of_workers - num_of_byz)] += logreg_grad(deepcopy(x), [A_i, y_i, l2, setting])
                else: 
                    grad_diff = logreg_grad(deepcopy(x), [A_i, y_i, l2, setting]) - logreg_grad(x1, [A_i, y_i, l2, setting])
                    w[i-(num_of_workers - num_of_byz)] += sparsificator(grad_diff, sparsificator_params)[0]
                    for j in range(n):
                        if agg == "GM":
    ...
    with open("dump/" + filename + "_Byz_MARINA_No_Sync_" + attack + "_" + agg + "_gamma_" + str(gamma) + "_l2_" + str(l2) + "_p_" + str(p) + "_epochs_" + str(T) + "_workers_" + str(num_of_workers) + "_byz_" + str(num_of_byz) + "_K_" + str(sparsificator_params[0]) + ".txt", 'wb') as file:
